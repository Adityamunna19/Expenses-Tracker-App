...
import { X, Landmark, CreditCard, Trash2, Plus, Loader2 } from 'lucide-react';
import toast from 'react-hot-toast';

// Notice we added `accounts = []` here. If App.jsx forgets to pass it, it defaults to empty instead of crashing!
export default function UserProfilePane({ user, accounts = [], onClose, onSuccess, darkMode, API_BASE }) {
  ...
  // Safe variables for user name/initial to prevent string method crashes
  const firstName = user?.user_metadata?.first_name || '';
  const lastName = user?.user_metadata?.last_name || '';
  const fullName = firstName ? `${firstName} ${lastName}`.trim() : 'Captain';
  const initial = firstName ? String(firstName).charAt(0).toUpperCase() : (user?.email ? String(user.email).charAt(0).toUpperCase() : 'P');
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md z-[200] shadow-2xl animate-in slide-in-from-right duration-300 overflow-y-auto ${panelBg}`}>
        ...
        <div className="p-8 pt-4 space-y-10">
          ...
          <div className={`p-6 rounded-[2rem] shadow-lg flex items-center gap-5 ${cardBg}`}>
            <div className="w-16 h-16 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xl font-black shadow-inner">
              {initial}
            </div>
            <div className="overflow-hidden">
              <h3 className="text-xl font-black truncate">{fullName}</h3>
              <p className="text-xs font-bold opacity-50 truncate">{user?.email || 'No email provided'}</p>
            </div>
          </div>
                ...
                <div key={acc.id} className={`p-5 rounded-2xl flex justify-between items-center shadow-sm border-2 border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-all ${cardBg}`}>
                  <div className="flex items-center gap-4">
                    <div className={`p-3 rounded-xl ${acc.type === 'card' ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600'}`}>
                      {acc.type === 'card' ? <CreditCard size={20} /> : <Landmark size={20} />}
                    </div>
                    <div>
